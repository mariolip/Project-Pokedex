...
if ($browser_rank = ) {
    ...
    $sql = "SELECT `id`,`image`,`a`,`b`,`c`,`d` FROM WhosThatPokemon where difficulty = ORDER BY rand() limit 1";   
    mysql_query($sql);
...
elseif ($browser_rank = ) {
    ...
    echo "Who's That Pokemon!"
    $sql = "SELECT `id`,`image`,`a`,`b`,`c`,`d` FROM WhosThatPokemon ORDER BY rand() limit 1";   
    mysql_query($sql);
    echo '<form action="Finish.php" method="post"><div>';
    while($data = mysql_fetch_array($result,MYSQL_NUM)){
     echo <<<EOF
